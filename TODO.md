...
- [ ] **Milestone notebook** — *still not done*. The transport surface has no worked example in `notebooks/12_steam_tables.ipynb` or a notebook of its own. It is the one deliverable this milestone shipped without, and it is deliberately **not** rolled into the release below
