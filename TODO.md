...
## Milestone 19: Petroleum Characterization *(~20–30h)* — **Code complete (unreleased)**
...
The largest new module; gated by nothing. **One sub-item shipped partial — the
Kesler–Lee `CF` term — and the checkbox says which.**

- [x] **Distillation-curve interconversion** — `engine/src/petroleum/distillation.rs`. `DistillationBasis` {D86, Tbp, D2887, Efv} + `DistillationCurve` (validated, monotone, interpolating both ways) + `convert_curve` routing any basis to any other through TBP. Point-wise power laws (Riazi–Daubert 1986; Edmister–Okamoto) *and* the API difference procedures 3A1.1 / 3A3.1 / 3A3.2, whose 50 %-anchored accumulation is written once and shared by all three. **All published-example tests pass to \< 0.15 °C** and every leg round-trips to 1e-8
- [x] **TBP cutting** — `engine/src/petroleum/cuts.rs`. `CutSpec::{EqualVolume, EqualTemperature, Boundaries}` — the third being a plan addition, because matching a refinery's product yields means cutting where the *tower* cuts. Cut `tb` is the exact volume-average of the piecewise-linear interpolant (panel edges include every interior knot), which differs from the midpoint value by \> 0.5 K on a half-barrel cut and by nothing on a narrow one
- [x] **Per-cut property estimation** — `engine/src/petroleum/properties.rs`. `PropertyMethod::{RiaziDaubert1980, ApiRiaziDaubert1987, KeslerLee, Twu}` → M, Tc, Pc, Vc; `acentric_lee_kesler` with both `Tbr` branches; `ZcMethod::{LeeKesler, Reid, Salerno, Nath}`. Twu's implicit reference molecular weight is inverted by bisection over a 16–5000 g/mol bracket and reports rather than extrapolating off it. `engine/src/petroleum/gravity.rs` carries API ↔ SG, Watson K, and the five average boiling points (API 2B1.1), with the `wabp ≥ vabp ≥ cabp ≥ meabp ≥ mabp` ordering asserted
- [x] **Fraction correlations — Maxwell–Bonnell** — `engine/src/petroleum/vapor_pressure.rs`: `normal_boiling_point` (the AET conversion vacuum distillation depends on), its exact inverse `boiling_point_at_pressure`, and `vapor_pressure` by Brent on `log₁₀ P` (**closed-form per branch since M20**, Brent kept as fallback and oracle). Two quirks of the published fit are **measured and asserted** rather than smoothed over: it is not an identity at 1 atm (+0.22 → +0.37 K), and `Q` steps 0.35–0.55 K across the 760 mmHg branch boundary
- [x] **Fraction correlations — ideal-gas Cp°** — `engine/src/petroleum/cp.rs`, Kesler–Lee / API 7D3.6, emitted directly as the `[f64; 5]` `Component.cp_coeffs` polynomial so a pseudocomponent needs no special case in `energy.rs`. **Partial: the `CF` non-paraffinic correction is not implemented** — its published coefficients could not be verified against a primary source, and shipping unverified constants in a thermodynamic correlation is worse than shipping a documented gap. Measured cost: paraffins 2.9 %, aromatics 3.1 %, **naphthenes up to 15.9 % low**. The test asserts the gap from *both* sides so implementing `CF` forces the docs to be updated
- [x] **PyO3 + wrapper + tests** — `engine/src/py_petroleum.rs` (`Assay` pyclass + 14 `petro_*` functions, string-based model selection, dict-returning cuts) and `python/src/vle/petroleum.py` (`Assay` with `cuts` / `components` / `to_system`, unit strings, API-gravity input). **37 wheel-level tests** in `python/tests/test_m19_petroleum.py`, including the Riazi Example 3.3 oracle re-checked through the binding
- [x] **📓 Milestone notebook** — `notebooks/15_petroleum_characterization.ipynb` (20th notebook at the time), executes top-to-bottom in a fresh kernel with numeric assertion cells pinning the published values
- [x] **Verification** — **121 new Rust unit tests** (engine lib 208 → 329, workspace 318 → 439); Python suite 457 → **494**; `cargo fmt --check` clean, `cargo +1.97.0 clippy --workspace --all-targets` 0 warnings; Chapter IV validation intact

## Milestone 20: Refinery Thermodynamics *(~18–26h)* — **Code complete (unreleased)**
...
Depends on M19 for the fractions these methods apply to. Built for the outer
loop of an inside-out column solver — every method O(N) per stage except the
one O(N²) Lee–Kesler mixing pass. **Two scope decisions stated, not rounded up.**

- [x] **Free-water / three-phase** — the **water-decant model** (the "at minimum" option): `engine/src/flash/free_water.rs::flash_free_water`. Case A: vapor saturated at `Pˢᵃᵗ_w/P`, dry hydrocarbon flash at `P − Pˢᵃᵗ_w` with the System's own models, free water by balance; case B (balance negative or `Pˢᵃᵗ_w ≥ P`): all water in the vapor, fixed point on `y_w`. Warm-started dry flashes, no re-indexing of the spec (the flash tolerates a zero-feed component). **Not** a three-liquid stability search — cannot find a second hydrocarbon liquid, neglects dissolved water; both stated in the module docs. 7 unit tests: drum decant, hot stripper, undersaturated vapor, all-liquid, explicit `Pˢᵃᵗ`, dry feed ≡ plain flash, guard rails
- [x] **Grayson–Streed + BK10** — `LiquidModel::{GraysonStreed, BraunK10}` in `engine/src/eos.rs` + `flash/system.rs`. GS = `regular_solution_ln_nu(GraysonStreed1963)` × Scatchard–Hildebrand γ (component `solubility_param`/`liquid_volume` or spec overrides; γ ≡ 1 if absent) ÷ vapor φ̂, ν and γ constants in `SystemTpCache`; the legacy `ChaoSeader` (GS table, no γ — corrected in the docs) kept unchanged; `RegularSolutionSet::ChaoSeader1961` added, all six coefficient rows verified against two independent transcriptions each. BK10 = Maxwell–Bonnell `ln Pˢᵃᵗ − ln P − ln φ̂ⱽ`, cached per (T, P), Watson-K correction from the new `Component::watson_k`; pressure-correction charts not implemented (stated). **`vapor_pressure` inverted in closed form** (quadratic in log₁₀ P per branch, < 1e-7 vs the Brent oracle over 500+ points; deterministic at the branch step) and `ln_vapor_pressure` extrapolates the outer branches for light ends. `k_values_with_derivs` reports `Unsupported` for all three regular-solution/chart models. 4 system tests + 2 eos tests + 4 vapor-pressure tests
- [x] **Lee–Kesler enthalpy departure** — `engine/src/refinery/lee_kesler.rs`: `lee_kesler_reduced` / `_departure` / `_pseudocritical` (η = 1 or 0.25) / `_departure_mix`; consistency identities `H = −Tr²∂lnφ/∂Tr` (2e-5) and `S = H/RT − lnφ` (1e-10) asserted on both fluids at six states, `dZ/dVr` vs FD, `B(Tr)` vs Pitzer, methane vs PR, N = 300 smoke. **Measured 0.10 ms per mixture enthalpy at N = 300** after replacing the inner-loop `powf` (0.31 ms → 0.09 ms, 3.4×). 14 unit tests
- [x] **Peneloux volume translation** — `engine/src/refinery/volume_translation.rs`: `peneloux_shift` (SRK / PR families, Z_RA from `Component::zra` or the ω correlation), `peneloux_shift_mix`, `translated_molar_volume`, `translated_liquid_density`. n-C7 SRK translated 691 vs 684 measured, n-C10 PR 721 vs 727 (raw SRK 596/598). 6 unit tests
- [x] **PyO3 + wrapper + tests + 📓 notebook** — `engine/src/py_refinery.rs` (3 functions + `RegularSolutionSet`), 7 `System` methods in `py_system.rs` (+ `mws`/`watson_ks`/`zras` constructor arrays, `liquid_kind` `grayson_streed`/`bk10`), `python/src/vle/refinery.py`, `System.flash_free_water` & co. in `system.py`, `FreeWaterFlashResult`/`LeeKeslerDeparture` in `results.py`, `Component.{solubility_param, watson_k, zra}` filled by `Assay.components()`. **13 wheel-level tests** (`test_m20_refinery.py`); `notebooks/16_refinery_thermodynamics.ipynb` (21st notebook) executes top-to-bottom with assertion cells
- [x] **Verification** — Rust workspace tests 439 → **477** (engine lib 329 → 359 — 30 new in `refinery/` + `free_water.rs`, 8 more in `flash/system.rs`, `eos.rs` and `vapor_pressure.rs`), Python suite 494 → **507**; `cargo fmt --check` clean, `cargo +1.97.0 clippy --workspace --all-targets` 0 warnings; Chapter IV validation intact
- [x] **Gaps §8 of `docs/en/petroleum/README.md` re-examined** — 8.3 narrowed by the closed-form inversion; 8.4 (Kesler–Lee `CF`) still open, now with the evidence recorded: two secondary sources found (DWSIM source, a 2025 *J. Chem. Pet. Eng.* review), they disagree on B₁'s exponent, and neither variant improves the naphthene error on measured pure-compound Cp° (one makes it worse); the SG-based API form was also evaluated
...
| 19. Petroleum Characterization | ~20–30h | **Code complete (unreleased)** — `engine/src/petroleum/` (7 submodules): D86 ↔ TBP ↔ D2887 ↔ EFV interconversion (Riazi–Daubert power laws + API difference procedures 3A1.1/3A3.1/3A3.2), TBP cutting by volume / temperature / product boundaries, four critical-property families (R-D 1980, API 1987, Kesler–Lee, Twu) + Lee–Kesler ω + four Zc correlations, Kesler–Lee ideal-gas Cp° and Maxwell–Bonnell. `Assay` → `list[Component]` drops straight into the flash. **121 Rust + 37 Python tests**; interconversions match Riazi (2005) Ex. 3.2–3.5 and two API TDB examples to < 0.15 °C. **Partial: the Kesler–Lee `CF` naphthene correction is not implemented** (measured cost up to 15.9 % on ring compounds), and Edmister–Okamoto's 0–10 % EFV row is documented as unusable on narrow feeds. Notebook 15. `PETROLEUM_PSEUDOCOMPONENT_PLAN.md` |
| 20. Refinery Thermodynamics | ~18–26h | **Code complete (unreleased)** — `engine/src/refinery/` (Lee–Kesler departure pure + mixture, Peneloux translation) + `flash/free_water.rs` (water-decant flash) + `LiquidModel::{GraysonStreed, BraunK10}` with ν / Maxwell–Bonnell constants cached per (T, P); Maxwell–Bonnell inverted in closed form. **37 Rust + 13 Python tests**; Lee–Kesler validated by thermodynamic identities, Peneloux against measured densities, 0.10 ms per N = 300 mixture enthalpy. **Stated scope**: free water is the decant model (no three-liquid stability search); BK10 has no pressure-correction charts. Notebook 16. `PETROLEUM_PSEUDOCOMPONENT_PLAN.md` |
