...
2. **Persistence notice** — a single-line blockquote markdown cell immediately
   after the title, warning users that the hub does **not** guarantee their
   edits will survive a container restart or image rebuild. Use this exact
   wording so the notice is consistent across notebooks:

   ```markdown
   > 💾 **Heads up — this hub is a sandbox.** Edits you make here may not
   > survive a container restart or a hub-image rebuild. If you change this
   > notebook and want to keep your work, use **File → Download** to save a
