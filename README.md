...
- **Crude assay → pseudocomponents**: a distillation curve plus a gravity becomes a list of ordinary `Component`s, so a barrel of crude flashes with the same code path a two-component mixture uses
...
the same bundle wrapped by Tauri, Electron or Capacitor). It also ships **21
...
tables, a 25-compound database, 477 Rust + 507 Python tests and 21 executable
notebooks. The remaining `0.` in the version number is about **API** stability,
not the numerics. The full engine (22+ EOS, mixing rules with exact derivatives, energy properties, and the modern flash/regression suite), the high-level `vle.System` Python API with a parallel batch numpy layer, and 21 notebooks — all validated against the thesis's Chapter IV tables. Latest release: **v0.16.0** on PyPI + crates.io — the **γ-φ heat-capacity release** (Milestone 12.6, opened by a downstream staged-separation (distillation) consumer): `System.phase_cp` / `flash::phase_cp` now cover γ-φ liquids and ideal-gas vapors as the exact temperature derivative of the shipped enthalpy, on top of dual-number-generic saturation correlations (`dPsat/dT` analytic for every model) and a per-model excess Cpᴱ; design record [GAMMA_PHI_CP_PLAN.md](docs/plans/engine/GAMMA_PHI_CP_PLAN.md). Before it, **v0.15.0** — the **petroleum and refinery release**. **Milestone 20 — refinery thermodynamics** (`vle_thermo::refinery` / `vle.refinery`, notebook `16_refinery_thermodynamics.ipynb`): a free-water decant flash for steam-stripped feeds, Grayson–Streed and Braun K10 K-value methods with a closed-form Maxwell–Bonnell inversion, Lee–Kesler enthalpy departure (pure + mixture, 0.10 ms at N = 300), and Peneloux volume translation — with the scope stated (decant model, not a three-liquid stability search; no BK10 pressure charts). **Milestone 19 — petroleum characterization** (`vle_thermo::petroleum` / `vle.petroleum`), which turns a crude assay into hundreds of pseudocomponents: D86 ↔ TBP ↔ D2887 ↔ EFV curve interconversion, TBP cutting, four critical-property correlation families from boiling point and gravity, Kesler–Lee ideal-gas Cp° and Maxwell–Bonnell vapor pressure, all landing as ordinary `Component`s that flash with no special casing (notebook `15_petroleum_characterization.ipynb`). One documented gap: the Kesler–Lee `CF` naphthene correction is not implemented, costing up to 15.9 % on ring compounds' Cp°. **v0.14.0** was an **engine performance release**: the mixture core is now **N-scalable**. Classical mixing with no binary interaction parameters (the normal state of a petroleum assay) takes an **O(N)** path instead of the unconditional O(N²) double loop; a sparse correction covers real non-zero pairs in O(N + nnz); the analytic composition Jacobian is a sum of rank-1 terms so `d_ln_phi_d_n_apply` computes `J·v` **without forming the N×N block**; Wong-Sandler's own quadratic collapses by a different identity; and `MixtureWorkspace` makes a composition sweep **allocation-free** after its first evaluation. Measured at N = 300: `ln_phi_mix` **30.7×**, the Jacobian **50.4×**, growth linear where it was quadratic — with the flash 18–24 % faster end-to-end than v0.12.0. Design record: [PETROLEUM_PSEUDOCOMPONENT_PLAN.md](docs/plans/engine/PETROLEUM_PSEUDOCOMPONENT_PLAN.md) §1.1 and [OPTIMIZATION_PLAN_PART2.md](docs/plans/engine/OPTIMIZATION_PLAN_PART2.md) §7. **v0.13.0** was a **steam release**: `vle-steam` gains **transport properties** (viscosity IAPWS R12-08, thermal conductivity R15-11 with the critical enhancement, surface tension R1-76(2014), plus derived Prandtl / kinematic viscosity / thermal diffusivity), surfaced as `vle.steam.viscosity`/`thermal_conductivity`/`surface_tension`/`transport` and as attributes on `steam.Water` and the saturation row — per-phase, never quality-averaged. The same release is an **IF97 performance pass**: a power-table series evaluation (~3.3× on every forward region), a safeguarded-Newton region-3 density solve (−90 %) that also corrected a latent bad density bound, and region-2 backward `T(p,s)` (−87.5 % on `ps_vapor`). The thermo engine is unchanged. **v0.12.1** was a documentation release (every plan and audit moved to [docs/plans/](docs/plans/README.md), with a [Plan & Audit History](docs/plans/README.md) as the entry point; no code change). **v0.12.0** was a performance release (isothermal flash −24…−28 %, tangent-plane stability −44…−51 %) responding to an external audit, with the measured verdicts and the rejected recommendations recorded in [OPTIMIZATION_PLAN_PART1.md](docs/plans/engine/OPTIMIZATION_PLAN_PART1.md) and [OPTIMIZATION_PLAN_PART2.md](docs/plans/engine/OPTIMIZATION_PLAN_PART2.md); **v0.10.0** shipped the steam tables. **Milestones 15–17** add the local-build app channels — **Swift** (iOS/macOS via UniFFI), **Kotlin** (Android + Compose Desktop via UniFFI; code complete, first Android Studio run pending), and **WebAssembly** (browser / Tauri / Electron / Capacitor via wasm-bindgen) — nothing published or committed as a binary, one build script each (`scripts/build-{ios,android,wasm}.sh`). **Milestone 14** adds the **NRTL** activity model (general multicomponent, with analytic ∂lnγ/∂T and excess enthalpy via dual-number AD) and **ammonia** to the bundled database (now 25 compounds) — the upstream liquid model a downstream staged-separation (distillation) consumer needs for the ammonia–water enthalpy–composition method (notebook `13_nrtl_ammonia.ipynb`). **Milestone 13** adds a new dependency-free crate **`vle-steam`** implementing the **IAPWS-IF97** industrial steam tables ("VLE for water only") — all five regions + the saturation line + backward equations, verified against the R7-97(2012) tables to 9 significant figures — surfaced as **`vle.steam`** with unit-aware inputs and a batch numpy API (notebook `12_steam_tables.ipynb`). **Milestone 12** — the downstream derivative & database release — is done across two releases: **v0.8.2** grew the bundled database to **24 compounds** with ideal-gas Cp°/R coefficients; **v0.9.0** added a Rust-side component database (`vle_thermo::db`, `component-db` feature), **exact temperature/pressure derivatives** of fugacity and K-values (dual-number AD), **real-mixture heat capacity** and **partial molar enthalpy**, and a packaged γ-φ phase enthalpy — the property surface a downstream staged-separation library consumes. **v0.9.1** is a patch fixing a ~1% Wong-Sandler departure-enthalpy inconsistency that the M12.3 Gibbs–Helmholtz invariant surfaced (root cause + fix in [DERIVATIVE_RELEASE_PLAN.md](docs/plans/engine/DERIVATIVE_RELEASE_PLAN.md) §7). See [DERIVATIVE_RELEASE_PLAN.md](docs/plans/engine/DERIVATIVE_RELEASE_PLAN.md). Per-milestone detail: [ROADMAP.md](ROADMAP.md).
...
- [Petroleum Characterization — A Learning Guide](docs/en/petroleum/README.md) — Why crude oil has no component list, and how a distillation curve plus a gravity becomes one: the four distillation bases, the Watson factor, the five average boiling points, every correlation written out with its published units, and every function in `vle_thermo::petroleum` / `vle.petroleum`
...
The implementation cites 41 academic references (ACS format). Key ones include:
...
|---|---|---|
| **Claude Opus 4.6** (1M context) | 1–3 | Foundation: legacy-code analysis, reference mapping, English translation, repo structure, the modernization plan |
| **Claude Opus 4.7** (1M context) | 4–7 | Units add-on, component database, CI/CD + publishing pipeline, numerics layer |
| **Claude Opus 4.8** (1M context) | 7–8.1, 10–14 | The full pure-component EOS zoo (22+ α variants, 3-parameter EOS, saturation models, virial) and the activity-coefficient models; then the Python wrapper and batch numpy API, the Chapter IV walkthrough, the downstream derivative & database release, the IAPWS-IF97 steam tables, and NRTL + ammonia |
| **Claude Fable 5** | 8.2–9, 15–17 | **The major performance & algorithm modernization**: the measured performance foundation (benches, allocation-free hot paths, caches), the generalized (A, B, U, W) mixture core with **exact analytic/dual-number derivatives** replacing every finite difference, mixture energy properties, and the complete modern flash suite — stability analysis, guaranteed-convergence Rachford-Rice, bubble/dew, adiabatic flash, Heidemann–Khalil critical points, phase-envelope continuation through the critical point, and kij/Aij regression — validated against the thesis's Chapter IV tables; later the three local-build app channels — Swift/iOS, Kotlin/Android and WebAssembly |
| **Claude Opus 5** (1M context) | 13.7–13.8, 18–19 | Steam transport properties and the IF97 performance pass; the **N-scalable mixture core** (O(N) classical mixing, rank-1 composition Jacobian, measured linear to N = 300); and **petroleum characterization** — assay → pseudocomponents |

In short: the project **started with Opus 4.6** doing the archaeology and planning; **Fable 5 delivered the deep optimization work** — the exact-derivative architecture and modern algorithm suite that the original thesis identified as its own main weaknesses (Ch. IV §4.1) — and **Opus 5 took the engine to scale**, first making the mixture core grow linearly rather than quadratically with component count, then building the petroleum layer that needs it.

*(This table is derived from the `*Executed by Claude Code using …*` lines in [`ROADMAP.md`](ROADMAP.md), which are the record of who did what. Milestone 9 records it in prose instead — "shipped (Claude Code using Claude Fable 5)" — and Milestones 0 and 20 carry no attribution: 0 predates the convention, and 20 is not started.)*
