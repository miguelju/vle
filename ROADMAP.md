...
- [ ] Tag a release (`v0.2.0`) — CI publishes to PyPI + crates.io; deploy-sandbox auto-fires the notebooks-only fast path on tag, the full image rebuild stays manual via `workflow_dispatch` + `full_deploy=true` *(M6.3 — pending operator approval before pushing the tag)*
